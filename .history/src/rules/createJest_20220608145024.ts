...
`;
    ...
    const localReg = /import\s+{(\s*[^}]+\s*)}\s+from\s+['"][^'"]+['"]/;
    const allAndLocalReg = /import\s+([^{}\s]+,)\s*{(\s*[^}]+\s*)}\s+from\s+['"][^'"]+['"]/;
    if(fileContext){
        ...
        if(allRes) {
            ...
            if(fnName) {
                importStr = `import ${allAndLocalRes[1]},{${allAndLocalRes[2]}, ${fnName}} from './${fileName}'`;
                const _fileContext = fileContext.replace(allReg, importStr);
                fs.writeFileSync(filePath, _fileContext);
            }
        }
